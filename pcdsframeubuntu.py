...
if __name__ == '__main__':
	...
	if (paramfile == '-g'):
		...
		sample.write('# Sample parameters generated for PCDSFrame software\n')
		sample.write('# \n')
		...
		sample.write('# Max cores to use\n')
		sample.write('cores=22\n\n')

		sample.write('# OpenMP cores per MPI process\n')
		sample.write('cores_per_mpi=6\n\n')

		sample.write('# MPI binding level: : socket, core:<n>\n')
		sample.write('bl=socket\n\n')

		sample.write('# MPI mapping policy: : scatter, compact\n')
...
# ##################################################################################
	...
	print ('\n************************************\n')
	print   ('*  The MS/MS Proteomics Pipeline   *\n')
	...
	with open(paramfile) as params:
		for line in params:
			...
			# Set the MPI binding level
				...
				print ('Using MPI bp =', val)

			# Set OMP cores per MPI
			elif (param == 'cores_per_mpi'):
				threads = int(val)
				if (threads <= 0 or threads > max_cpus):
					threads = int(max_cpus)
...
# ##################################################################################
	...
	if (autotune == 1):
		...
		if (os.path.isfile(workspace + '/autogen/info.out') == False):
			...
			autotune = call('lscpu | tr -d " \\r" && numactl --hardware | tr -d " \\r" > ' + workspace + '/autogen/info.out 2>&1', shell=True)

		# Wait for the lscpu process to complete 
		while (os.path.isfile(workspace + '/autogen/info.out') == False):
			time.sleep(0.5)
		...
		with open(workspace + '/autogen/info.out') as minfo:
			for line in minfo:
				...
				if (param == 'Socket(s)'):
					...
					print ('Cores/socket  =', cores_per_socket)

				elif (param == 'NUMAnode(s)'):
					numa = int(val)
					print ('NUMA nodes =', numa)
		...
		if (newparams == True or os.path.isfile(workspace + '/autogen/counter.out') == False):
			...
			autotune3 = call(pcdsframepath + '/counter/counter.exe > ' + workspace + '/autogen/counter.out 2>&1', shell=True)

		# Wait for the counter process to complete
		while (os.path.isfile(workspace + '/autogen/counter.out')):
			time.sleep(0.5)
...
# ######################## APPLY OPTIMIZATIONS ##########################################################
		...
		print('Setting Cores/MPI =', threads)
		print('Setting nMPI =', mpi_per_node)
...
# ##################################################################################
	...
	if (mpi_per_node > 1):
		if (bp[:5] == 'core:' and int(bp[5:]) != threads):
			bp = bp[:5] + str(threads)
	else:
		bp = 'socket'

	# Run the PCDSFrame software
	cfir = call('mpirun -np ' + str(mpi_per_node) + ' -bind-to ' + bl + ' -map-by ' + bp + ' ' + pcdsframepath + '/cfirindex/cfir.exe ' + uparams', shell=True)

	print ('The output is written at: '+ workspace + '/output\n\n')
