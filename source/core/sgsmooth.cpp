...
private:
    ...
    float_mat &operator =(const float_mat &)
    {
        return *this;
    }
    ;
...
// copy constructor for matrix
float_mat::float_mat(const float_mat &m) :
        std::vector<float_vect>(m.size())
{
...
static int partial_pivot(float_mat &A, const size_t row, const size_t col, float_vect &scale,
        int_vect &idx, double tol)
{
    ...
    if(piv_elem < tol)
    {
        printf("partial_pivot(): Zero pivot encountered.\n")
#endif
...
static int lu_factorize(float_mat &A, int_vect &idx, double tol = TINY_FLOAT)
{
    ...
    {
        printf("lu_factorize(): cannot handle empty "
    ...
    for (i = 0; i < (int) A.nr_rows(); ++i)
    {
        ...
        if (maxval == 0.0)
        {
            printf("lu_factorize(): zero pivot found.\n");
            return 0;
        }
        scale[i] = 1.0 / maxval;
    }
...
float_mat operator *(const float_mat &a, const float_mat &b)
{
    ...
    {
        printf("incompatible matrices in multiplication\n");
        return res;
    }
 ...
 * used. */
void sg_smooth(lwvector<double> *v, lwvector<double> *res, const int width, const int deg)
{
    /* Results vector */
    /* Constants */
    ...
    if (deg == 0)
    {
#if 0 /* No need of OpenMP as already multithreaded */
#if defined(_OPENMP)
#pragma omp parallel for private(i,j) schedule(static)
#endif
#endif
        // handle border cases first because we need different coefficient
        for (i = 0; i < width; ++i)
        {
            const double scale = 1.0 / double(i + 1);
            float_vect c1(width, scale);
            for (j = 0; j <= i; ++j)
            {
                (*res)[i] += c1[j] * (*v)[j];
 ...
 * result. */
float_vect sg_derivative(const float_vect &v, const int width, const int deg, const double h)
{
    float_vect res(v.size(), 0.0);
    if ((width < 1) || (deg < 1) || ((int) v.size() < (2 * width + 2)))
    {
        printf("sgsderiv: parameter error.\n");
        return res;
    }
    ...
    float_vect c(lsqr_fprime(b, deg));
